...
## 2026-08-18 · 会话: T-014 失败分析面板 bug 修复（乙）

### 修复
- [x] detail-row colspan 错位：5→6（表格新增对比列后展开行少占一列）
- [x] 点击失败记录滚动居中：block nearest→center（原 nearest 在元素视口内时不滚动，点击像"没反应"）
- [x] 全零结果状态误判"成功"：badge() 与对比面板 _cmpStatus 新增全零识别，显示紫色「全零」标签（与失败分析面板口径一致）

### 验证
- 本机 /dashboard 手动测试：25 条混合记录分类准确（超时 9 / 退出码非0 10 / 全零 8），成功记录不误归类，点击展开+滚动居中正常
