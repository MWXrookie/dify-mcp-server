...
- [ ] T-005 工作流含校验节点
- [ ] T-006 50 次测试成功率 ≥ 90%
- [ ] `git tag phase-1-complete` 已打标签

---

## 4. 阶段二：智能体闭环（Week 3-5）

**目标**：把后端能力封装为可对话的 Web 产品。用户打开浏览器就能用。

### 4.1 任务清单

```
T-007 [P1] 结果分析 MCP 工具 ──── 无依赖 ────→ T-011
T-008 [P1] 结果解释 + 审查节点 ── 无依赖 ────→ T-011
T-009 [P1] 迭代循环（工作流改造）── 无依赖 ──→ T-011
                                                    │
T-010 [P1] Gradio Web 前端 ────── T-007 ───────→ T-011
                                                    │
T-011 [P1] 多轮对话记忆 ───────── T-007~T-010 ──→ T-012
                                                    │
...
| 属性 | 值 |
|------|-----|
| 输入 | 仿真 stdout（含压力场数据）、ARCHITECTURE.md 第 4.1.4 节 |
...
| 可用 Agent | 1 个 |
| 验证 | 执行一次仿真后，工具返回 heatmap_base64 非空、max_pressure 正确 |
| 完成标准 | 每次成功仿真自动生成热力图 PNG + 物理量摘要 |
...
- [ ] analyze_simulation_result 工具上线
- [ ] 每次仿真自动生成热力图
...
| T-006 50 次测试 | 🔄 进行中 (82%, p0修复后~92%) | 2026-08-08 | fecb882, 3faaa39 |
